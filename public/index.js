...
function loginUser() {
    ...
    .then(response => {
        // Check if the response is successful (i.e., user exists)
        if (response.ok) {
            return response.text();  // Expecting the password as plain text
    ...
    .then(dbPassword => {
        // Compare the password from the input with the one from the database
